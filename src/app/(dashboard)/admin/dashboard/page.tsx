  ...
  return (
    <div className="space-y-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-zinc-900 dark:text-white mb-1">Platform Command</h1>
          <p className="text-zinc-500 font-medium">Real-time overview of your multi-vendor ecosystem.</p>
        </div>
        <div className="flex items-center gap-3">
          <Button variant="outline" className="rounded-xl border-zinc-200 dark:border-zinc-800 bg-zinc-100/50 dark:bg-zinc-900/50 hover:bg-zinc-200 dark:hover:bg-zinc-800">
            Export Logs
          </Button>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat: any, i: any) => (
          <Card key={i} className="bg-white dark:bg-zinc-900/30 border-zinc-200 dark:border-zinc-800 overflow-hidden relative group">
      ...
      <div className="grid gap-6 lg:grid-cols-12">
        {/* Main Chart Area */}
        <Card className="lg:col-span-8 bg-white dark:bg-zinc-900/40 border-zinc-200 dark:border-zinc-800 shadow-xl dark:shadow-2xl">
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <Badge variant="outline" className="border-zinc-200 dark:border-zinc-800 text-zinc-500">Yearly</Badge>
            </div>
          </CardHeader>
          <CardContent>
            <div className="h-[350px] w-full flex flex-col justify-end gap-2 pt-10">
              <div className="flex items-end justify-between h-full gap-2 px-2">
                {[40, 65, 45, 90, 75, 55, 85, 40, 60, 80, 50, 70].map((h: number, i: number) => (
                  <div key={i} className="flex-1 group relative">
                    <div 
                      className="w-full bg-zinc-200 dark:bg-zinc-800 group-hover:bg-emerald-500/40 transition-all rounded-t-sm" 
                      style={{ height: `${h}%` }}
                    />
                    {i === 3 && (
              ...
              <div className="flex justify-between px-2 text-[10px] font-bold text-zinc-400 dark:text-zinc-600 uppercase tracking-tighter mt-4">
        ...
        {/* Sidebar Activity */}
        <Card className="lg:col-span-4 bg-white dark:bg-zinc-900/40 border-zinc-200 dark:border-zinc-800 shadow-xl dark:shadow-2xl overflow-hidden">
          <CardHeader>
          ...
          <CardContent className="space-y-6">
            <div className="p-4 rounded-xl bg-emerald-500/5 border border-emerald-500/10 flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-[10px] text-zinc-500 dark:text-zinc-400">Next maintenance in 12 days</p>
              </div>
            </div>

            <div className="space-y-4">
              <h5 className="text-[10px] font-black uppercase tracking-widest text-zinc-400 dark:text-zinc-600">Active Sessions</h5>
              {[
            ...
            <Button variant="ghost" className="w-full rounded-xl text-zinc-500 hover:text-zinc-900 dark:hover:text-white hover:bg-zinc-100 dark:hover:bg-zinc-800/50 text-xs font-bold uppercase border border-dashed border-zinc-200 dark:border-zinc-800 h-10 mt-4">
      ...
      {/* Vendor Table Area */}
      <Card className="bg-white dark:bg-zinc-900/40 border-zinc-200 dark:border-zinc-800 shadow-xl dark:shadow-2xl overflow-hidden">
        <CardHeader className="flex flex-row items-center justify-between border-b border-zinc-200 dark:border-zinc-900 pb-6">
          <div>
          ...
          <div className="flex items-center bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg px-3 py-1.5 text-zinc-500 w-64">
            <Search size={14} />
        ...
        <CardContent className="p-0">
            ...
            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-900">
              {recentVendors.map((vendor: any, i: number) => (
                <tr key={i} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/20 transition-colors group">
                  <td className="px-6 py-5 font-bold text-zinc-900 dark:text-zinc-100">{vendor.name}</td>
                  <td className="px-6 py-5 text-zinc-500 dark:text-zinc-400 font-medium">{vendor.owner}</td>
                  <td className="px-6 py-5 underline underline-offset-4 decoration-emerald-500/20 text-zinc-700 dark:text-zinc-300">{vendor.plan}</td>
                  <td className="px-6 py-5 text-emerald-500 font-bold tracking-tighter">{vendor.revenue}</td>
                  <td className="px-6 py-5 text-center">
                    <Badge className={
                      vendor.status === 'Active' 
                        ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20 hover:bg-emerald-500/20' 
                        : 'bg-zinc-100 dark:bg-zinc-500/10 text-zinc-600 dark:text-zinc-500 border-zinc-200 dark:border-zinc-500/20 hover:bg-zinc-200 dark:hover:bg-zinc-500/20'
                    }>
                      {vendor.status}
                    </Badge>
                  </td>
                  <td className="px-6 py-5 text-right opacity-0 group-hover:opacity-100 transition-opacity">
                    <button className="p-2 hover:text-zinc-900 dark:hover:text-white text-zinc-400 dark:text-zinc-500 transition-colors">
          ...
          <div className="p-4 border-t border-zinc-200 dark:border-zinc-900 text-center">
